...
async def reset(message: Message) -> None:
    """эвакуация из беседы!!! шутка. Сброс базы данных..."""
    ...
    try:
        ...
        await message.answer(
            "я не понял кто ты, админ или нет "
            + "потому что я сам не админ :(("
        )
        return
    ...
    if from_id in admins:
        ...
        await message.answer(f"@id{from_id}, заебись, база сброшена")
    else:
        await message.answer(
            "ты не админ ёпта, тебе не положено"
        )
